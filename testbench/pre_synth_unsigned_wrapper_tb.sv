`timescale 1ns / 1ps
module pre_synth_unsigned_wrapper_tb;
  // Basic parameters
  parameter bits = 2;
  parameter rows = 3;
  parameter columns = 4;
  ...
  reg unsigned [realStep-1:0] TheoreticalResult[A_rows-1:0][columns-1:0];
  integer iterationPractical = 0;
  integer flag = 0;
  
  // Temporary B & A element arrays
  reg unsigned [bits-1:0] Bfull[0:rows-1][0:columns-1];
  reg [columns-1:0] Bin[bits-1:0][rows-1:0];
  reg unsigned [bits-1:0] Afull[0:A_rows-1][0:A_columns-1];
  ...
  initial begin
    ...
    for (i = 0; i < A_rows; i = i + 1) begin
      for (j = 0; j < A_columns; j = j + 1) begin
        Afull[i][j] = $urandom_range(0, (1<<bits) - 1);
        //Afull[i][j] = (1<<bits) - 1;
      end
    end
    ...
    for (i = 0; i < rows; i = i + 1) begin
      for (j = 0; j < columns; j = j + 1) begin
        Bfull[i][j] = $urandom_range(0, (1<<bits) - 1);
        //Bfull[i][j] = (1<<bits) - 1;
      end
    end
